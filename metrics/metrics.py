...
def _detect_label_key(results_list: list) -> str:
    """Return the interestingness label key used in this results list."""
...
def run_metrics(
    ...
    results_list, kept_labels, dropped_labels, label_counts = filter_results_by_min_support(
        results_list, label_key=label_key, min_frac=min_frac
    )
    ...
    labels_all = np.array([r[label_key] for r in results_list])
