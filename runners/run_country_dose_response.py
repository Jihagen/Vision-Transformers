"""
Runner: Branch C2 -- wide alpha-sweep dose-response for the Nigeria country
vector @ language_23_D5120, on the BLANK (no-persona) prompt.

Motivation: Branch C's primary (fixed +-2 / ablate) and secondary
(male_excitement, alpha in -2..2) experiments both showed only a small
rating-distribution shift (~5x smaller than Branch B's interest vector at the
same alpha range) and zero lexicon-count shift (nigeria/african words never
appeared). Before concluding the country direction is causally weak, sweep a
much wider alpha range on the confound-free blank prompt -- mirroring Branch
B's primary design (which showed a real Delta=0.247 effect at alpha=+-2) -- to
check for a threshold/non-linear effect at larger injection magnitude.

Measures (per alpha):
    - rating-distribution (mean score, dose-response curve)
    - lexicon-count shift (nigeria/nigerian/african/africa/... words)
    - JSON-parse failure rate (rating == "?"), as a sanity check that large
      alpha doesn't simply break the output format rather than shift content

Usage
-----
Dry-run (check paths, no model load):
    python runners/run_country_dose_response.py --dry_run

Full run (defaults: language_23_D5120, n=100, alphas -8 -4 -2 -1 0 1 2 4 8):
    python runners/run_country_dose_response.py
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Branch C2: wide alpha-sweep dose-response for the Nigeria "
                     "country vector on the blank prompt."
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Target layer for injection (default: language_23_D5120).",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--offload_suffix", default="country_dose_response",
        help="Suffix for CPU offload directory (default: country_dose_response).",
    )
    p.add_argument(
        "--output_dir", default=None,
        help="Directory for results (default: results/country_validation/<layer>/dose_response_blank).",
    )
    p.add_argument(
...
def _resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    return Path("results/country_validation") / layer_slug / "dose_response_blank"
...
def main() -> None:
    ...
    print(f"\n✅ Branch C2 country dose-response complete -> {out_dir}")
