"""
Runner: wide alpha-sweep dose-response for the emotion attack vector actually
used by the excited_vs_angry UAP (avg_excitement_vs_avg_anger @ language_29,
the UAP's default layer), on the BLANK (no-persona) prompt.

Mirrors run_workload_dose_response.py / run_country_dose_response.py /
run_interest_dose_response.py exactly, including the same default alpha list,
so this is directly comparable to the interest and workload dose-response
curves already collected. Existing per-emotion "primary" checks
(emotion_validation/language_29/{excitement,anger}) used different vectors
(single-emotion one-vs-rest, fixed +-2/ablate only) — this is the first real
alpha-sweep on the actual combined attack vector.

Usage
-----
Dry-run (check paths, no model load):
    python runners/run_emotion_attack_dose_response.py --dry_run

Full run (defaults: language_29_D5120, n=100, alphas -8 -4 -2 -1 0 1 2 4 8):
    python runners/run_emotion_attack_dose_response.py
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Wide alpha-sweep dose-response for the excitement-anger "
                     "emotion attack vector on the blank prompt."
    )
    p.add_argument(
        "--vector_path", default=str(_DEFAULT_VECTOR),
        help="Path to the .npy mean-difference vector dict (default: excited_vs_angry attack vector).",
    )
    p.add_argument(
        "--layer", default=_DEFAULT_LAYER,
        help=f"Target layer for injection (default: {_DEFAULT_LAYER}, the UAP's default layer).",
    )
    p.add_argument(
        "--lexicon_json", default=None,
        help="Path to logit-lens evidence JSON with a 'lexicon' field. Default: "
             "results/extra_checks/logit_lens/excited_vs_angry_<layer>.json, matching "
             "--layer (produced by run_emotion_attack_logit_lens.py).",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--offload_suffix", default="excited_vs_angry_dose_response",
        help="Suffix for CPU offload directory.",
    )
    p.add_argument(
        "--output_dir", default=None,
        help="Directory for results (default: results/emotion_validation/<layer>/excited_vs_angry/dose_response_blank).",
    )
    p.add_argument(
...
def _resolve_output_dir(args: argparse.Namespace, layer: str) -> Path:
    ...
    return Path("results/emotion_validation") / layer_slug / "excited_vs_angry" / "dose_response_blank"
...
def main() -> None:
    ...
    lexicon_json = Path(args.lexicon_json) if args.lexicon_json else \
        _LEXICON_DIR / f"excited_vs_angry_{layer_slug}.json"
    ...
    logger.info(f"Loading excited_vs_angry vector @ {layer}")
    vec = np.load(args.vector_path, allow_pickle=True).item()
    ...
    print(f"\n✅ excited_vs_angry dose-response complete -> {out_dir}")
